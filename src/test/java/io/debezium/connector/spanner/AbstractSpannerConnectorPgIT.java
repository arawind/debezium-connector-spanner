...
public class AbstractSpannerConnectorPgIT extends AbstractConnectorTest {

    private static final KafkaEnvironment KAFKA_ENVIRONMENT = new KafkaEnvironment(
            KafkaEnvironment.DOCKER_COMPOSE_FILE);
    protected static final Database database = Database.builder()
            .generateDatabaseId()
            .dialect(Dialect.POSTGRESQL).build();
    ...
    public static int waitTimeForRecordsAfterNulls() {
        return Integer.parseInt(System.getProperty(TEST_PROPERTY_PREFIX + "records.waittime.after.nulls", "10"));
    }
